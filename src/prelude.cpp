...
int main()
{
	...
	Maybe<int> m1;
	Maybe<int> m3(7);
	Maybe<Function<int, int>> mf(Applicative<Maybe>::pure<Function<int, int>>(increment));
	Maybe<Function<int, int>> m2 = Applicative<Maybe>::pure<Function<int, int>>(increment);
	std::cout << "Maybe Int: " << Maybe<int>(3) << std::endl;
	std::cout << "Nothing < Just 5: " << (m1 < m) << std::endl;
	std::cout << "Nothing || Just 5: " << (m1 || m) << std::endl;
	std::cout << "Just increment << Nothing: " << (mf << m1) << std::endl;
	std::cout << "Just increment << Just 5: " << (mf << m) << std::endl;
	std::cout << "Nothing >>= Just increment: " << (m >>= mf) << std::endl;
	std::cout << "Just 5 >>= Just increment: " << (m1 >>= mf) << std::endl;
